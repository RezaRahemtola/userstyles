...
- **The danger is messaging a TERMINATED agent, not a live one.** `SendMessage` to a killed agent does not error — the harness restarts it with no transcript and *your message as its new prompt*, so it opens a browser and writes theme files. That is the only rogue-writer path. Never message an agent after you `TaskStop` it.
- **Do message a LIVE agent that went idle without a summary.** An `idle_notification` with no report means it is waiting, not gone. `SendMessage` it for the report, wait for the reply, then verify on disk and `TaskStop` — in that order. Reconstructing findings from `git diff` and mtimes is a fallback for a silent agent, not the default: it recovers what changed but never *why*, so caveats, unfixables and self-flagged doubts are lost. On the 2026-08-01 patrol nearly every teammate was read off disk this way; the ones that did report surfaced three real bugs (an invented yellow badge, a two-tone logo inverting to a white box, a light mobile app-banner) that no diff would have revealed.
...
- **Verify LIVENESS from evidence** (`TaskStop`'s running list, `stat`, `css-hash.sh`, `git`, `npx playwright-cli list`) — never ping to find out whether something is alive. Once you know it *is* alive, asking it for its report is correct.
- **Kill in the same tool block as the verification.** The moment a teammate's report is in hand: verify its work on disk AND kill it in one block. Every miss on the 2026-07-09 patrol came from batching reports and deferring the kill.
