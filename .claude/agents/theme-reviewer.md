...
3. **Run the SEMANTIC scanner on every page type** — this is the one that finds the dominant bug class:
   ```bash
...
5. **VISUAL PASS — mandatory, and you do it, not the promoter.** A clean scanner run is NOT evidence: on the 2026-07-09 patrol both scanners returned 0/0/0 on pages carrying six real bugs, and the two ship-blockers of the whole run (genius blanking the lyrics column when an annotation opened; khan showing no indicator for the selected answer) were found by *looking at rendered pixels*, never by a scan. Before you declare done, screenshot every page type at full length and **open the image and look at it**. Specifically:
